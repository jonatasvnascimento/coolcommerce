...
function Checkout(props) {
    ...
    const [shippingStreet, setShippingStreet] = useState(
        dev ? 'Rua Fake Strit 123' : ''
    );
    const [shippingPostalZipCode, setShippingPostalZipCode] = useState(
        dev ? '07914080' : ''
    );
    const [shippingCity, setShippingCity] = useState(dev ? 'São Paulo' : '');
    const [shippingStateProvince, setShippingStateProvince] = useState(
        dev ? '' : ''
    );
    const [shippingCountry, setShippingCountry] = useState(dev ? 'SP' : '');
                ...
                return (
                    <>
                        ...
                        <FormControl className={classes.formControl}>
                            <InputLabel id="shippingCountry-label">Cidade</InputLabel>
                            <Select
